...
t2 = np.linspace(300, 700, 400) * 1e-3
...
with Bar('    * progress: ', max=len(t2), suffix='%(percent)d%%') as bar:
    for f in t2:
        ...
        def fun_time(x, f=f, y=year, r=receptor, q=quantity):
            def inner_fun(xx):
                ...
                # field = double_pulse(peak1, t1, f * xx[0], peak2, t2, (-peak1/peak2 * f * xx[0]), period, time)
                field = double_pulse(peak1, t1, tau1, f * xx[0], tau2, period, time)
...
# plot results
hf = plt.figure()
plt.plot(t2 * 1e3, mu[:, 0], label='time')
plt.fill_between(t2 * 1e3, mu[:, 0] - ci_factor*sigma[:, 0], mu[:, 0] + ci_factor*sigma[:, 0], color="C0", alpha=0.2)
plt.plot(t2 * 1e3, mu[:, 1], label='frequency')
plt.fill_between(t2 * 1e3, mu[:, 1] - ci_factor*sigma[:, 1], mu[:, 1] + ci_factor*sigma[:, 1], color="C1", alpha=0.2)
plt.plot(t2 * 1e3, mu[:, 2], label='frequency proposed')
plt.fill_between(t2 * 1e3, mu[:, 2] - ci_factor*sigma[:, 2], mu[:, 2] + ci_factor*sigma[:, 2], color="C2", alpha=0.2)
lg = plt.legend(fontsize=14)
lg.set_draggable(True)
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.xlabel('$t_2$ (ms)', fontsize=16)
...
np.savez_compressed('./parametric_data/double_pulse', t2=t2,
                    mu=mu, sigma=sigma)
